#!/usr/bin/env python3
"""
WAYPOINT PILOT - L-Shaped Path with RSRP Logging

Flies through waypoints following an L-shaped path to avoid dead zone.
Logs all connectivity metrics for comparison with RL agent.

Path: (0, 140, 25) -> waypoints -> (140, 10, 25) [L-SHAPED avoiding dead zone]
"""
...
class WaypointPilot(Node):
    def __init__(self):
        super().__init__('waypoint_pilot')
        
        # ============ SCENARIO (L-shaped path) ============
        self.source = np.array([0.0, 140.0, 25.0])
        self.destination = np.array([140.0, 10.0, 25.0])
        
        # Waypoints for L-shaped path (avoiding center dead zone)
        self.waypoints = [
            np.array([10.0, 140.0, 25.0]),   # Start near source
            np.array([50.0, 139.0, 25.0]),   # Move east along top
            np.array([80.0, 135.0, 25.0]),   # Continue east
            np.array([108.0, 120.0, 25.0]),  # Turn southeast
            np.array([135.0, 90.0, 25.0]),   # Move south
            np.array([140.0, 50.0, 25.0]),   # Continue south
            np.array([140.0, 10.0, 25.0])    # Destination
        ]
        
        self.current_waypoint_idx = 0
        self.WAYPOINT_THRESHOLD = 8.0  # Distance to consider waypoint reached
    ...
    def _print_startup(self):
        ...
        self.get_logger().info("WAYPOINT PILOT - L-Shaped Path (with RSRP logging)")
        self.get_logger().info("=" * 60)
        ...
        self.get_logger().info(f"Direct distance: {distance:.1f}m (Following L-shaped path)")
        self.get_logger().info("=" * 60)
        ...
        for transform in msg.transforms:
            if 'X3' in transform.child_frame_id:
                ...
                if not self.position_received:
                    ...
                    self.get_logger().info("Position received, starting baseline flight...")
    ...
    def control_loop(self):
        ...
        distance_to_goal = np.linalg.norm(self.current_pos[:2] - self.destination[:2])
        ...
        if self.current_waypoint_idx < len(self.waypoints):
            ...
            distance_to_waypoint = np.linalg.norm(self.current_pos[:2] - target[:2])
            ...
            # All waypoints reached, head to final destination
            target = self.destination
        
        # Fly toward current target
        direction = target[:2] - self.current_pos[:2]
        ...
        if self.step_count % 25 == 0:
            progress_pct = 100 * (1 - distance_to_goal / np.linalg.norm(self.destination - self.source))
            ...
            self.get_logger().info(
                ...
                f"Dist:{distance_to_goal:.0f}m ({progress_pct:.0f}%) | "
    ...
    def complete_mission(self, success):
        ...
        # Print summary
        self.get_logger().info("\n" + "=" * 60)
        self.get_logger().info(f"BASELINE MISSION: {'SUCCESS' if success else 'TIMEOUT'}")
        self.get_logger().info("=" * 60)
        ...
        filepath = f'/app/logs/waypoint_results_{timestamp}.json'
